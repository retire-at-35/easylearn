...
import { getSingleQuestionList, addOrUpdateSingleQuestion, deleteSingleQuestion } from '@/api/question'
...
const fetchQuestionList = async () => {
  ...
  try {
    ...
    const res = await getSingleQuestionList(params)
...
const handleDelete = (row) => {
  ElMessageBox.confirm(
    ...
    try {
      await deleteSingleQuestion(row.id)
...
const handleSubmit = () => {
  questionFormRef.value?.validate(async (valid) => {
    if (valid) {
      try {
        await addOrUpdateSingleQuestion({
          id: questionForm.id,
